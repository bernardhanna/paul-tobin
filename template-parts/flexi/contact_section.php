...
<section id="<?php echo esc_attr($section_id); ?>" class="flex relative bg-[#f9fafb]">
  <div class="w-full max-w-[80rem] mx-auto">
    <div class="grid grid-cols-1 md:grid-cols-2 gap-[3rem] pt-[2.5rem] lg:pt-[5rem] pr-[5rem] pb-[2.5rem] lg:pb-[5rem] pl-[5rem] max-xl:px-5 bg-[#f9fafb] h-auto w-full <?php echo $padding_class_str; ?>">

      <!-- LEFT COLUMN -->
      <div class="flex flex-col md:flex-col md:justify-start md:items-start gap-[1.5rem]  max-w-full self-start w-full">
        <div class="flex flex-col md:flex-col md:justify-center md:items-start gap-[1.5rem] self-start">
            ...
            <?php
              echo cs_build_heading(
                $l_tag,
                $l_text,
                " max-w-full text-left text-[2.125rem] font-[600] leading-[2.5rem] tracking-[-0.01rem] text-[#0a1119] font-secondary max-sm:text-[2.125rem] max-sm:font-semibold max-sm:leading-[2.5rem]"
              );
            ?>
            <div class="flex flex-row justify-between items-start w-[4.4375rem] h-[0.3125rem] self-start">
              <div class="h-[0.3125rem] bg-[#ef7b10] w-full"></div>
              <div class="h-[0.3125rem] bg-[#0098d8] w-full"></div>
              <div class="h-[0.3125rem] bg-[#b6c0cb] w-full"></div>
              <div class="h-[0.3125rem] bg-[#74af27] w-full"></div>
            </div>
          
        </div>

        <?php if (!empty($l_desc)) : ?>
          <div class="wp_editor  max-w-full text-left text-[1.125rem] font-[500] leading-[1.75rem] tracking-[0.0625rem] text-[#4d4d4d] font-primary">
        ...
        <?php endif; ?>
          ...
          <?php
            echo cs_button_from_link(
              $l_link,
              'flex-col md:flex-row md:justify-center md:items-center pr-[2rem] pl-[2rem] bg-[#0f172a] btn inline-flex justify-center items-center gap-2 hover:opacity-90 transition-opacity duration-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 px-8 py-3.5 min-h-12 lg:h-[2.75rem] lg:min-h-0 h-[2.75rem] hover:bg-[#40bff5] self-center w-full text-white hover:text-black',
              'Book now via Calendly'
            );
          ?>
      

        <div class="flex flex-col self-center w-full md:flex-col md:justify-center md:items-center">
          <?php
            if ($l_media === 'calendly') {
      ...
      <div class="flex flex-col md:flex-col md:justify-start md:items-start gap-[1.5rem]  max-w-full self-start w-full">
        <div class="flex flex-col md:flex-col md:justify-center md:items-start gap-[1.5rem] self-start">
            <?php
              echo cs_build_heading(
                $r_tag,
                $r_text,
                " max-w-full text-left text-[2.125rem] font-[600] leading-[2.5rem] tracking-[-0.01rem] text-[#0a1119] font-secondary"
              );
            ?>
            <div class="flex flex-row justify-between items-start w-[4.4375rem] h-[0.3125rem] self-start">
              <div class="h-[0.3125rem] bg-[#ef7b10] w-full"></div>
        ...
        <?php endif; ?>
          ...
          <?php
            echo cs_button_from_link(
              $r_link,
              'flex-col md:flex-row md:justify-center md:items-center pr-[2rem] pl-[2rem] border-[0.125rem] border-[rgba(10,17,25,1)] btn inline-flex justify-center items-center gap-2 hover:opacity-90 transition-opacity duration-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 px-8 py-3.5 min-h-12 lg:h-[2.75rem] lg:min-h-0 h-[2.75rem] hover:bg-[#40bff5] self-center w-full',
              'Request a call'
            );
          ?>
