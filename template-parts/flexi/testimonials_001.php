...
$is_kind_words_page     = is_page('kind-words');
...
$testimonials = [];
if (!$is_kind_words_page && $testimonial_source === 'manual') {
    if (is_array($selected_testimonials) && !empty($selected_testimonials)) {
        foreach ($selected_testimonials as $testimonial_id) {
            ...
            if (!$testimonial_post || $testimonial_post->post_type !== 'testimonial' || $testimonial_post->post_status !== 'publish') {
                continue;
            }
            $manual_excerpt = has_excerpt($testimonial_post) ? get_the_excerpt($testimonial_post) : '';
            $testimonials[] = [
                'name'        => $testimonial_post->post_title,
                'title'       => get_post_meta($testimonial_post->ID, 'job_title', true) ?: $manual_excerpt,
                'testimonial' => apply_filters('the_content', $testimonial_post->post_content),
            ];
        }
    } elseif ($manual_testimonials) {
        $testimonials = $manual_testimonials;
    }
} else {
    $posts = get_posts([
        'post_type'        => 'testimonial',
        'posts_per_page'   => $is_kind_words_page ? -1 : $number_of_testimonials,
        'post_status'      => 'publish',
        'orderby'          => 'date',
        'order'            => 'DESC',
    ...
    foreach ($posts as $post) {
        $manual_excerpt = has_excerpt($post) ? get_the_excerpt($post) : '';
        $testimonials[] = [
            'name'        => $post->post_title,
            'title'       => get_post_meta($post->ID, 'job_title', true) ?: $manual_excerpt,
                ...
                <?php if ($is_kind_words_page) : ?>
                    <div class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:gap-8">
                        <?php foreach ($testimonials as $t) : ?>
                            <?php
                            $testimonial_raw = isset($t['testimonial']) ? (string) $t['testimonial'] : '';
                            $title_raw       = isset($t['title']) ? (string) $t['title'] : '';

                            // Prevent duplicate rendering when title/excerpt equals testimonial body text.
                            $testimonial_compare = trim(preg_replace('/\s+/', ' ', wp_strip_all_tags(html_entity_decode($testimonial_raw, ENT_QUOTES, 'UTF-8'))));
                                    ...
                                    <div class="flex relative z-10 flex-col gap-4">
                                        <?php if (!empty($testimonial_raw)) : ?>
                                            <div class="flex flex-row">
                                                <div class="mt-2 text-base font-normal tracking-normal leading-6 text-[#0A1119] font-primary">
                                                    <?php echo wp_kses_post($testimonial_raw); ?>
                                                </div>
